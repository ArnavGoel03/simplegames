...
- `packages/cards/src/rounds.ts` (`cardsInRound`, `trumpForRound`)
- `packages/cards/src/deal.ts` (`dealRound`)

## Regenerating

Needed whenever the Chaupal side of any of those five files changes. Until the
...
test that imports the modules above by path, replays the three seeds in
...
5 and 13 (the first, the one the home page shows, and the last), and the hands
are a four seat round five deal of nine cards each, which is what
`Commitment.tsx` puts on screen. The `rounds` vector records the hand size and
trump of every round from the real implementation. It catches ladder changes
that a shuffle fixture with a hardcoded hand size cannot detect.
